...
def cpuschedu():
    ...
    def PB():
        def sub():
            ...
            pro_data.sort(key=lambda x: x[1])
            '''
            Sort proes according to the Arrival Time
            '''
            for i in range(len(pro_data)):
                ready_queue = []
                temp = []
                ...
                if len(ready_queue) != 0:
                    ...
                    '''
                    Sort the proes according to the Priority, considering Higher the Value, Higher the Priority
                    '''
                    start_time.append(s_time)
            ...
            while 1:
                ...
                for i in range(len(pro_data)):
                    if pro_data[i][1] <= s_time and pro_data[i][3] == 0:
                        ...
                        if len(ready_que) != 0:
                            for k in range(len(ready_que)):
                                if pro_data[i][0] == ready_que[k][0]:
                                    present = 1
                        '''
                        The above if loop checks that the next pro is not a part of ready_que
                        '''
                        if present == 0:
                            ...
                            temp = []
                        '''
                        The above if loop adds a pro to the ready_que only if it is not already present in it
                        '''
                        ...
                        '''
                        The above if loop makes sure that the recently executed pro is appended at the end of ready_que
                        '''
                    elif pro_data[i][3] == 0:
                        temp.extend([pro_data[i][0], pro_data[i][1], pro_data[i][2], pro_data[i][4]])
                        normal_que.append(temp)
                ...
                if len(ready_que) != 0:
                    if ready_que[0][2] > time_slice:
                        '''
                        If pro has remaining burst time greater than the time slice, it will execute for a time period equal to time slice and then switch
                        '''
                        start_time.append(s_time)
                        ...
                        '''
                        If a pro has a remaining burst time less than or equal to time slice, it will complete its execution
                        '''
                        start_time.append(s_time)
                        s_time = s_time + ready_que[0][2]
                        e_time = s_time
